...
export function PaymentCheckoutModal({
  ...
  try {
    ...
    // In a real app, we'd fetch this from the student's profile or program
    // For this demo, we can try to find a school or default to null (which might break RLS if strict)
    // Let's look for a generic school or the one from the invite ref if stored
    let schoolId = null;

    // Try to get school from student profile if possible, otherwise use a fallback or letting it be null (if allowed)
    // But we need it for the dashboard.
    // We'll try to fetch the first school found for demo purposes if not passed props
    ...
    if (selectedMethod === 'transfer') {
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          school_id: schoolId // Link to school!
        });
    ...
    // We still insert into DB as 'paid' for the demo to show up in history
