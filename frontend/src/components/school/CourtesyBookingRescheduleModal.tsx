...
import { useFacilityReservations, type FacilityReservation, type CourtesySlot } from '@/hooks/useFacilityReservations';

const SLOTS_HORIZON_DAYS = 14;

export interface CourtesyBookingRescheduleModalProps {
  reservation: FacilityReservation | null;
  onClose: () => void;
}

export function CourtesyBookingRescheduleModal({ reservation, onClose }: CourtesyBookingRescheduleModalProps) {
  const { getCourtesySlots, rescheduleReservation, isRescheduling } = useFacilityReservations();

  const [loadingSlots, setLoadingSlots] = useState(false);
  const [slots, setSlots] = useState<CourtesySlot[]>([]);
  const [selectedSlot, setSelectedSlot] = useState<CourtesySlot | null>(null);
  ...
  useEffect(() => {
    ...
    getCourtesySlots(reservation.facility_id, from, to)
      .then(setSlots)
      .finally(() => setLoadingSlots(false));
  }, [reservation]);

  const slotsByDate = useMemo(() => {
    const groups: Record<string, CourtesySlot[]> = {};
  ...
  const handleConfirm = async () => {
    ...
    await rescheduleReservation({
      ...
      reservation_date: selectedSlot.slot_date,
  ...
  return (
    <Dialog open={!!reservation} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto bg-card text-card-foreground border-border/40 backdrop-blur-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl font-bold">
            <CalendarClock className="h-6 w-6 text-primary" /> Reprogramar Reserva
          </DialogTitle>
          <DialogDescription className="text-muted-foreground/80">
            Misma cancha — solo cambia la fecha/hora. Se le avisa al prospecto por correo.
          </DialogDescription>
        </DialogHeader>
        ...
        {loadingSlots ? (
          ...
          <div className="text-center py-10 text-sm text-muted-foreground">
            No hay horarios disponibles para esta cancha en los próximos {SLOTS_HORIZON_DAYS} días.
          </div>
        ) : (
              ...
              <div key={date} className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {daySlots.map((s) => {
                    ...
                    return (
                      <Button
                        key={`${s.slot_date}-${s.slot_start_time}-${s.facility_availability_id}`}
