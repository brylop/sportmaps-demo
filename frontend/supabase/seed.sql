...
SELECT v.id::uuid, v.email, v.full_name, v.role, v.phone, v.avatar_url
FROM (VALUES
...
) AS v(id, email, full_name, role, phone, avatar_url)
WHERE NOT EXISTS (SELECT 1 FROM spm_users WHERE email = v.email);
...
SELECT v.id::uuid, v.email, v.full_name, v.role, v.avatar_url
FROM (VALUES
...
) AS v(id, email, full_name, role, avatar_url)
WHERE NOT EXISTS (SELECT 1 FROM spm_users WHERE email = v.email);

-- ATHLETE PROFILES
INSERT INTO spm_players (id, user_id, nickname, birthdate, position, height_cm, weight_kg, dominant_foot)
SELECT v.id::uuid, v.user_id::uuid, v.nickname, v.birthdate::date, v.position, v.height_cm, v.weight_kg, v.dominant_foot
FROM (VALUES
    -- Football
    ...
    ('50000000-0000-0000-0000-000000000003', '40000000-0000-0000-0000-000000000003', 'Rayo', '2011-03-10', 'Mediocampista', 150, 42, 'Derecho'),
    ('50000000-0000-0000-0000-000000000004', '40000000-0000-0000-0000-000000000004', 'Tomy', '2013-11-05', 'Portero', 148, 40, 'Derecho'),
    ('50000000-0000-0000-0000-000000000005', '40000000-0000-0000-0000-000000000005', 'Samu', '2012-01-30', 'Lateral', 142, 36, 'Derecho'),
    ('50000000-0000-0000-0000-000000000006', '40000000-0000-0000-0000-000000000006', 'Jero', '2014-06-12', 'Extremo', 138, 33, 'Izquierdo'),
    ...
    ('50000000-0000-0000-0000-000000000008', '40000000-0000-0000-0000-000000000008', 'Mari', '2010-09-22', 'Fondo', 158, 48, NULL),
    ('50000000-0000-0000-0000-000000000009', '40000000-0000-0000-0000-000000000009', 'Isa', '2013-04-18', 'Artístico', 140, 35, NULL),
    ('50000000-0000-0000-0000-000000000010', '40000000-0000-0000-0000-000000000010', 'Gaby', '2011-07-07', 'Velocidad', 152, 43, NULL),
    ('50000000-0000-0000-0000-000000000011', '40000000-0000-0000-0000-000000000011', 'Anto', '2014-12-01', 'Fondo', 136, 31, NULL),
    ('50000000-0000-0000-0000-000000000012', '40000000-0000-0000-0000-000000000012', 'Lu', '2012-10-30', 'Artístico', 144, 38, NULL)
) AS v(id, user_id, nickname, birthdate, position, height_cm, weight_kg, dominant_foot)
WHERE NOT EXISTS (SELECT 1 FROM spm_players WHERE user_id = v.user_id::uuid);
