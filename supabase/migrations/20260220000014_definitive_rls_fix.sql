...
BEGIN
    ...
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'school_branches' AND schemaname = 'public') LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || r.policyname || '" ON public.school_branches';
    END LOOP;
END $$;
...
CREATE POLICY "branches_select" ON public.school_branches FOR SELECT 
USING (true);

CREATE POLICY "branches_manage" ON public.school_branches FOR ALL
USING (public.fn_is_admin_of_school(school_id))
WITH CHECK (public.fn_is_admin_of_school(school_id));
