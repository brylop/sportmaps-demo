-- =============================================================================
-- 20260829100621_trial_class_self_create_enrollment_unico_activo.sql
-- Autor: judegor99   Fecha: 2026-08-29   Versión anterior: 20260829100110
-- Objetivo: fix encontrado probando la repetición sin tope (20260829100110)
--   contra la base real — no es especulativo.
--
--   Tres índices únicos parciales sobre enrollments — uq_enrollment_child_plan,
--   uq_enrollment_unregistered_plan, idx_enrollments_user_offering_plan_active
--   (uno por columna de identidad) — impiden dos enrollments 'active' para la
--   misma persona en el mismo offering_plan_id. Cada categoría de prueba
--   reusa SIEMPRE el mismo offering_plan_id lazy-creado (una sola vez, la
--   primera vez que se agenda esa categoría) — así que la SEGUNDA prueba de
--   la misma persona en la misma categoría choca al crear su propio
--   enrollment, con 23505.
--
--   No es un bug nuevo de esta feature: es una condición que el flujo del
--   owner también podía disparar (reagendar el mismo prospecto en la misma
--   categoría), pero nadie la había ejercitado hasta ahora — trial_class_
--   update_status nunca cierra el enrollment al marcar 'realizada'/'no_show',
--   así que un enrollment de 1 sesión ya consumida se queda 'active' para
--   siempre. No se toca esa función (inmutable, y el owner no repite
--   categoría hoy); el fix va acá, donde sí hace falta.
--
--   Fix: antes de crear el enrollment nuevo, cerrar como 'completed'
--   cualquier enrollment 'active' previo de la MISMA persona en el MISMO
--   offering_plan_id — es exactamente lo que es: una prueba de 1 sesión que
--   ya se dio antes. 'completed' es un valor válido del CHECK de status.
-- =============================================================================
...
BEGIN
  ...
  -- FIX 20260829100621: una repetición reusa el mismo offering_plan_id de la
  -- categoría — si la prueba anterior de esta misma persona en esta misma
  -- categoría sigue 'active' (nunca se cierra sola), el INSERT de abajo
  -- choca con uq_enrollment_child_plan / uq_enrollment_unregistered_plan /
  -- idx_enrollments_user_offering_plan_active. Se cierra como 'completed' —
  -- es exactamente lo que es, una prueba de 1 sesión ya dada.
