-- =============================================================================
-- 20260829102825_trial_class_self_bloquear_si_ya_tiene_plan.sql
-- Autor: judegor99   Fecha: 2026-08-29   Versión anterior: 20260829100621
-- Objetivo: decisión de producto — desde Mis Inscripciones, antes de dejar
--   pedir una clase de prueba para un sujeto (hijo YA registrado, o el
--   propio adulto vía p_self), hay que chequear si ese sujeto YA tiene un
--   plan real activo en la escuela. Si lo tiene, no se le ofrece prueba —
--   ya es miembro, agenda sus clases desde su plan (ScheduleClassModal, que
--   ya existe). El chequeo es por ESCUELA completa, sin importar la
--   disciplina (misma decisión que trial_class_self_is_first, confirmada de
--   nuevo en conversación). Para un hermano/a NUEVO (unregistered_athlete_id)
--   no aplica — por definición no puede tener un plan todavía.
--
--   "Plan real" = un enrollment activo cuyo offering NO es 'single_session'
--   — ese tipo es exclusivo de los planes $0/1-clase que lazy-init-ean
--   trial_class_self_create/trial_class_create_booking/cortesía. Un plan de
--   verdad es 'membership' o 'session_pack' (los dos únicos tipos reales
--   verificados en la base viva, 2026-08-29).
-- =============================================================================
...
-- ============================================================
-- 1. ¿Este sujeto ya tiene un plan real y activo en la escuela?
--    Reusable desde el BFF para avisar ANTES de dejar elegir sujeto+categoría
--    (misma idea que trial_class_self_is_first para el preview de precio).
-- ============================================================
...
-- ============================================================
-- 2. trial_class_self_create: rechaza child_id/self con plan real activo.
--    Firma sin cambios respecto a 20260829100621.
-- ============================================================
...
BEGIN
  ...
  IF p_child_id IS NOT NULL THEN
    ...
    END IF;
    IF public.trial_class_self_has_active_plan(p_school_id, p_child_id, NULL) THEN
      RAISE EXCEPTION 'has_active_plan: % ya tiene un plan activo en esta escuela — agendá sus clases desde su plan, no como prueba', v_child.full_name;
    END IF;
  ...
  ELSIF p_self THEN
    IF public.trial_class_self_has_active_plan(p_school_id, NULL, p_created_by) THEN
      RAISE EXCEPTION 'has_active_plan: ya tenés un plan activo en esta escuela — agendá tus clases desde tu plan, no como prueba';
    END IF;
