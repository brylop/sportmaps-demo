-- ─────────────────────────────────────────────────────────────────────────────
-- school_athletes: con plan activo, team_monthly_fee = 0 SIEMPRE (equipo = roster).
--
-- Bug: cuando el equipo y el plan viven en la MISMA inscripción (Fase A: una sola
-- fila con team_id + offering_plan_id y un único monthly_fee), la vista calculaba
-- team_monthly_fee = COALESCE(e.monthly_fee, 0) de esa fila — pero ese monthly_fee
-- ES la cuota del plan (p.ej. 180.000). Resultado: el editor mostraba
-- "Mensualidad equipo = 180.000" y al poner 0 reaparecía el valor del plan
-- (el bloque de plan del PUT reescribe la misma fila).
--
-- Regla de producto (ya vigente en los cobros): si hay plan activo, el equipo es
-- SOLO roster y NO cobra. Por tanto team_monthly_fee debe ser 0 cuando hay plan,
-- sin importar el monthly_fee compartido de la fila. Sin plan, se respeta la cuota
-- del equipo (override del enrollment > precio del catálogo).
--
-- Único cambio vs 20260729000002: las 3 ramas (child/adult/unregistered) devuelven
-- 0 en team_monthly_fee cuando existe plan activo. Todo lo demás es idéntico.
-- ─────────────────────────────────────────────────────────────────────────────
