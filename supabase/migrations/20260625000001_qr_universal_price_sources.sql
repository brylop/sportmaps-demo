-- ============================================================
-- SPORTMAPS — QR universal: la fuente de precio sale de la CONFIG de la escuela
-- Propósito:
--   Una escuela se organiza por `business_model` (schools.business_model):
--     • teams → cobra por pertenecer a un equipo  → precio = teams.price_monthly
--     • plans → cobra por membresía/paquete         → precio = subscription_plans.price
--               (sessions_included NULL = ilimitado; N = paquete de N sesiones;
--                billing_period = periodicidad)
--     • both  → ambos conviven
--   El QR ya NO obliga al admin a teclear precios ni a elegir manualmente la
--   "fuente": LEE el business_model y ofrece equipos y/o planes según corresponda.
--   Encima, un `fixed_amount` opcional sobrescribe el precio (campañas/promos).
--
--   Prioridad de precio (server-side, el cliente no fija montos):
--     fixed_amount(promo) > plan.price > teams.price_monthly > p_monthly_fee > 0
--
--   Conserva: cover_image_url, hijo existente, link /payments-automation,
--   XOR del enrollment, precio de equipo (20260624000002).
-- Fecha: 2026-06-25
-- ============================================================

BEGIN;

-- ── 1. Schema: target_type 'plan' + fixed_amount (promo) ────────────────────
...
COMMENT ON COLUMN public.school_join_qr_codes.fixed_amount IS
    'Monto fijo (promo/campaña) que SOBRESCRIBE el precio derivado de equipo/plan. '
    'NULL = usar el precio de la fuente (equipo o plan).';


-- ── 2. get_join_qr_public: business_model + plans[] + target plan + fixed ────
...
BEGIN
    ...
    ELSIF v_qr.target_type = 'plan' AND v_qr.target_id IS NOT NULL THEN
        SELECT jsonb_build_object(
            'kind', 'plan', 'id', sp.id, 'name', sp.name, 'description', sp.description,
            'monthly_fee', sp.price, 'billing_period', sp.billing_period,
            'sessions_included', sp.sessions_included, 'plan_type', sp.plan_type
        ) INTO v_target
        FROM public.subscription_plans sp
        JOIN public.vendor_profiles vp ON vp.id = sp.vendor_profile_id
        JOIN public.schools s          ON s.owner_id = vp.user_id
        WHERE sp.id = v_qr.target_id AND s.id = v_qr.school_id;
    ...
    -- Equipos elegibles (solo si la escuela trabaja por equipos) ------------
    IF v_qr.target_type IN ('open', 'branch') AND v_business_model IN ('teams', 'both') THEN
    ...
    -- Planes elegibles (solo si la escuela trabaja por planes) --------------
    IF v_qr.target_type = 'open' AND v_business_model IN ('plans', 'both') THEN
        SELECT COALESCE(jsonb_agg(jsonb_build_object(
            'id', sp.id, 'name', sp.name, 'description', sp.description,
            'price_monthly', sp.price, 'billing_period', sp.billing_period,
            'sessions_included', sp.sessions_included, 'plan_type', sp.plan_type
        ) ORDER BY sp.price), '[]'::jsonb)
        INTO v_plans
        FROM public.subscription_plans sp
        JOIN public.vendor_profiles vp ON vp.id = sp.vendor_profile_id
        JOIN public.schools s          ON s.owner_id = vp.user_id
        WHERE s.id = v_qr.school_id
...
-- ── 3. create_school_join_qr: + p_fixed_amount (reemplaza firma 11-arg) ─────
...
-- ── 4. list_school_join_qrs: nombre del plan + fixed_amount ─────────────────
    ...
    FROM (
        SELECT
            ...
            CASE qr.target_type
                ...
                WHEN 'plan'   THEN (SELECT name FROM public.subscription_plans  WHERE id = qr.target_id)
                ELSE NULL
            END AS target_name
...
-- ── 5. submit_qr_signup: + p_plan_id; precio server-side con prioridad ──────
--    fixed_amount(promo) > plan.price > teams.price_monthly > p_monthly_fee
...
BEGIN
    ...
    -- Precio del plan (validando que el plan pertenece a esta escuela) ------
    IF v_plan_id IS NOT NULL THEN
        SELECT sp.price INTO v_plan_price
        FROM public.subscription_plans sp
        JOIN public.vendor_profiles vp ON vp.id = sp.vendor_profile_id
        JOIN public.schools s          ON s.owner_id = vp.user_id
        WHERE sp.id = v_plan_id AND s.id = v_school_id AND sp.is_active = true;
...
-- ── 6. list_school_plans: planes del catálogo de la escuela (para el panel) ──
--    El admin no puede armar el join vendor_profiles→school client-side por RLS.
...
BEGIN
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        'billing_period', sp.billing_period, 'sessions_included', sp.sessions_included,
