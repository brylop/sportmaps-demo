-- ─────────────────────────────────────────────────────────────────────────────
-- school_athletes: la mensualidad NO se suma (equipo + plan) — el plan manda.
--
-- Bug: un atleta con equipo (180.000) y plan (180.000) se mostraba con
-- mensualidad 360.000 en el listado y en la ficha, mientras el motor de cobros
-- (fix "cobro único por atleta: plan manda, equipo=roster") solo genera UN
-- cobro. La vista contradecía a los cobros reales.
--
-- Cambios:
--   1. price_monthly      → plan manda; sin plan, la cuota del equipo. Sin suma.
--   2. team_monthly_fee   → con plan activo NO hereda teams.price_monthly
--      (equipo = roster). Sin plan sigue heredando el precio del equipo, para
--      escuelas/flujos que cobran por equipo.
--   3. Backfill: cuotas de equipo de atletas con plan activo → 0 (no se cobran).
-- ─────────────────────────────────────────────────────────────────────────────
...
SELECT c.id,
    ...
    COALESCE(( SELECT
             CASE WHEN EXISTS ( SELECT 1
                  ...
                  -- Con plan activo el equipo es SOLO roster: no hereda el precio
                  -- del catálogo (solo respeta un override explícito).
                  THEN COALESCE(e.monthly_fee, 0::numeric)
    ...
    ( SELECT t.sport
        ...
        CASE
            -- Plan manda: si hay plan activo, la mensualidad es la del plan
            -- (override del enrollment > precio del plan). El equipo es roster.
            WHEN (EXISTS ( SELECT 1
              ...
              WHERE e.child_id = c.id AND e.status = 'active'::text AND e.offering_plan_id IS NOT NULL
             LIMIT 1), 0::numeric)
            -- Sin plan: manda la cuota del equipo (override > precio del equipo).
            WHEN (EXISTS ( SELECT 1
               FROM enrollments ep
...
UNION ALL
 SELECT pr.id,
    ...
    COALESCE(( SELECT
             CASE WHEN EXISTS ( SELECT 1
                      FROM enrollments ep
                     WHERE ep.user_id = pr.id AND ep.status = 'active'::text AND ep.offering_plan_id IS NOT NULL)
                  -- Con plan activo el equipo es SOLO roster: no hereda el precio
                  -- del catálogo (solo respeta un override explícito).
                  THEN COALESCE(e.monthly_fee, 0::numeric)
                  ELSE COALESCE(e.monthly_fee, t.price_monthly, 0::numeric)
    ...
    ( SELECT t.sport
        ...
        CASE
            ...
            WHEN (EXISTS ( SELECT 1
               FROM enrollments ep
              WHERE ep.user_id = pr.id AND ep.status = 'active'::text AND ep.offering_plan_id IS NOT NULL)) THEN COALESCE(( SELECT COALESCE(e.monthly_fee, op.price) AS "coalesce"
               FROM enrollments e
                 JOIN offering_plans op ON op.id = e.offering_plan_id
              WHERE e.user_id = pr.id AND e.status = 'active'::text AND e.offering_plan_id IS NOT NULL
             LIMIT 1), 0::numeric)
            ...
            WHEN (EXISTS ( SELECT 1
               FROM enrollments ep
              WHERE ep.user_id = pr.id AND ep.status = 'active'::text AND ep.team_id IS NOT NULL)) THEN COALESCE(( SELECT COALESCE(e.monthly_fee, t.price_monthly) AS "coalesce"
               FROM enrollments e
                 JOIN teams t ON t.id = e.team_id
              WHERE e.user_id = pr.id AND e.status = 'active'::text AND e.team_id IS NOT NULL
             LIMIT 1), 0::numeric)
...
-- ── Backfill: el equipo no cobra si el atleta tiene plan activo ──────────────
-- Deja en 0 la cuota de los enrollments de equipo cuando el mismo atleta tiene
-- un plan activo en la misma escuela. Sin impacto monetario: esos cobros ya se
-- cancelan por "plan manda"; lo que se corrige es la cuota fantasma que la UI
-- mostraba (y sumaba) en el editor del atleta.
DO $$
DECLARE
  v_count integer;
BEGIN
  UPDATE public.enrollments et
     SET monthly_fee = 0,
         updated_at  = now()
   WHERE et.status = 'active'
     AND et.team_id IS NOT NULL
     AND et.offering_plan_id IS NULL
     AND COALESCE(et.monthly_fee, 0) <> 0
     AND EXISTS (
           SELECT 1
             FROM public.enrollments ep
            WHERE ep.status = 'active'
              AND ep.offering_plan_id IS NOT NULL
              AND ep.school_id IS NOT DISTINCT FROM et.school_id
              AND (
                    (et.child_id                IS NOT NULL AND ep.child_id                = et.child_id)
                 OR (et.user_id                 IS NOT NULL AND ep.user_id                 = et.user_id)
                 OR (et.unregistered_athlete_id IS NOT NULL AND ep.unregistered_athlete_id = et.unregistered_athlete_id)
                  )
         );
  GET DIAGNOSTICS v_count = ROW_COUNT;
  RAISE NOTICE 'Cuotas de equipo puestas en 0 (atleta con plan activo): %', v_count;
END $$;
