-- ============================================================================
-- MIGRACIÓN: Sincronizar lógica de onboarding (Backend/Frontend)
-- Fecha: 2026-03-17
-- Descripción: Expande get_onboarding_status para incluir todos los flags 
--              necesarios para el checklist dinámico.
-- ============================================================================
...
DECLARE
  ...
  v_result jsonb;
BEGIN
  -- Obtener rol y estado de verificación de email desde auth.users (vía auth.jwt())
  SELECT role INTO v_role FROM public.profiles WHERE id = v_user_id;
  
  -- Verificar si el email está confirmado en Supabase Auth
  -- (requiere acceso a auth.users o usar el helper de JWT)
  ...
  SELECT jsonb_build_object(
    ...
    'profile_complete', (
        SELECT (
            ...
            (role != 'athlete' OR bio IS NOT NULL)
        ) 
    ...
    'has_enrollment', (
        SELECT EXISTS(
            ...
            WHERE s.parent_id = v_user_id AND e.is_active = true
        ) OR EXISTS(
            -- Para atletas (usando el perfil como estudiante si aplica)
            SELECT 1 FROM public.enrollments e
            JOIN public.students s ON e.student_id = s.id
            WHERE s.profile_id = v_user_id AND e.is_active = true
        )
    ),
    ...
    -- Flags Misceláneos
    'has_sports_interest', (
        SELECT (bio IS NOT NULL AND length(bio) > 10) 
        FROM public.profiles 
        WHERE id = v_user_id
    )
  ) INTO v_result;
