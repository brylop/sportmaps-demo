...
CREATE TABLE IF NOT EXISTS public.events (
  ...
  description        text,
  sport              text                  NOT NULL,
  creator_role       text                  NOT NULL CHECK (creator_role IN ('owner','organizer')),
...
DROP FUNCTION IF EXISTS public.EXISTS (SELECT 1 FROM school_members WHERE school_id = uuid AND profile_id = auth.uid() AND role IN ('owner', 'admin', 'coach') AND status = 'active') CASCADE;
DROP FUNCTION IF EXISTS public.EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin') CASCADE;
...
CREATE OR REPLACE FUNCTION public.EXISTS (SELECT 1 FROM school_members WHERE school_id = p_school_id uuid AND profile_id = auth.uid() AND role IN ('owner', 'admin', 'coach') AND status = 'active')
RETURNS boolean
LANGUAGE sql SECURITY DEFINER STABLE AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.school_members
...
CREATE OR REPLACE FUNCTION public.EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
RETURNS boolean
LANGUAGE sql SECURITY DEFINER STABLE AS $$
  SELECT EXISTS (
