-- =============================================================================
-- 20260829121405_trial_class_self_has_active_plan_ambiguedad.sql
-- Autor: judegor99   Fecha: 2026-08-29   Versión anterior: 20260829121154
-- Objetivo: bug real, encontrado probando el flujo completo después de
--   20260829115629 — esa migración AGREGÓ un overload de 4 parámetros a
--   trial_class_self_has_active_plan en vez de reemplazar el de 3, dejando
--   los dos coexistiendo. trial_class_self_create llama a esa función con
--   3 argumentos posicionales donde el último es un NULL sin tipar
--   (`trial_class_self_has_active_plan(p_school_id, p_child_id, NULL)`) —
--   con dos overloads candidatos (el de 3 exacto, o el de 4 con el cuarto
--   default), Postgres no puede resolver cuál llamar:
--
--     ERROR 42725: function ...(uuid, uuid, unknown) is not unique
--
--   Esto rompía TODO trial_class_self_create para sujeto child_id o self
--   (el camino new_sibling no llama a esta función, por eso no se notó
--   antes) — encontrado en la prueba de regresión de esta misma sesión,
--   antes de que nadie lo usara en producción real.
--
--   Fix: un solo overload (4 parámetros). Se dropea el de 3, y
--   trial_class_self_create pasa los 4 argumentos explícitos.
-- =============================================================================
...
-- ============================================================
-- 1. Un solo overload — se dropea el de 3 parámetros.
-- ============================================================

DROP FUNCTION IF EXISTS public.trial_class_self_has_active_plan(uuid, uuid, uuid);

-- ============================================================
-- 2. trial_class_self_create: las dos llamadas pasan los 4 argumentos
--    explícitos, sin ambigüedad. Firma de la RPC sin cambios.
-- ============================================================
...
BEGIN
  ...
  IF p_child_id IS NOT NULL THEN
    ...
    END IF;
    IF public.trial_class_self_has_active_plan(p_school_id, p_child_id, NULL, NULL) THEN
  ...
  ELSIF p_self THEN
    IF public.trial_class_self_has_active_plan(p_school_id, NULL, p_created_by, NULL) THEN
