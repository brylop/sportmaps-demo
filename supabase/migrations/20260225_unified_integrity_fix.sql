...
-- Eliminamos todas las políticas previas para evitar conflictos de nombres y lógica duplicada.
DO $$
DECLARE
    pol record;
BEGIN
...
RETURNS boolean LANGUAGE sql SECURITY DEFINER STABLE AS $$
  SELECT EXISTS (SELECT 1 FROM public.school_members WHERE profile_id = auth.uid() AND school_id = p_school_id AND role IN ('owner','admin','coach') AND status = 'active');
$$;
...
-- 4. RE-HABILITAR RLS EN TODAS LAS TABLAS
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY[
    'profiles','user_roles','user_search_preferences','schools','school_settings','school_branches',
    'school_members','school_staff','facilities','facility_reservations','programs','classes',
    'enrollments','class_enrollments','children','attendance_records','teams','team_members',
    'training_sessions','session_attendance','training_plans','match_results','academic_progress',
    'payments','payment_reminders','products','carts','orders','order_items','messages',
    'message_attachments','announcements','notifications','activities','calendar_events',
    'reviews','events','event_registrations','wellness_appointments','wellness_evaluations',
    'health_records','athlete_stats','training_logs'
  ]
  LOOP
...
-- PROFILES
CREATE POLICY "Profiles: select own or admin" ON public.profiles FOR SELECT USING (auth.uid() = id OR public.is_platform_admin());
...
CREATE POLICY "Settings: manage admin" ON public.school_settings FOR ALL USING (public.is_school_admin(school_id));
CREATE POLICY "Settings: select staff" ON public.school_settings FOR SELECT USING (school_id = ANY(public.user_school_ids()));
...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    'profiles','schools','school_settings','school_branches','school_members','school_staff',
    'facilities','facility_reservations','programs','classes','teams','children','enrollments',
    'payments','products','orders','activities','calendar_events','reviews','event_registrations'
  ]
  LOOP
...
-- RPC: Obtener estado de onboarding (Equipos en lugar de Programas)
...
BEGIN
  ...
  -- Identificar contexto de escuela
...
-- RPC: Aceptar Invitación (Sincronización de Perfil y Roles)
...
BEGIN
    ...
    -- 1. Sincronizar Rol Global en Profiles
    -- Buscar el role_id correspondiente al nombre
