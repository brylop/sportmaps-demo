...
SELECT 
    school_id,
    COUNT(id) FILTER (WHERE payment_status = 'paid') as transactions_count,
    COALESCE(SUM(amount) FILTER (WHERE payment_status = 'paid'), 0) as total_collected,
    COALESCE(SUM(amount) FILTER (WHERE payment_status IN ('pending', 'pending_approval')), 0) as total_outstanding,
    COALESCE(SUM(amount) FILTER (WHERE payment_status = 'overdue'), 0) as total_at_risk,
    CASE 
        WHEN SUM(amount) > 0 THEN 
            ROUND((SUM(amount) FILTER (WHERE payment_status = 'paid') * 100.0 / SUM(amount)), 2)
        ELSE 0 
...
GROUP BY ar.student_id, s.id, s.school_id
...
SELECT 
    ...
    (
        ...
        WHERE e.program_id = p.id AND e.is_active = true
    ) as active_enrollments,
    COALESCE((
        SELECT SUM(t.amount)
        FROM public.transactions t
        JOIN public.enrollments e ON t.enrollment_id = e.id
        WHERE e.program_id = p.id AND t.payment_status = 'paid'
    ), 0) as total_revenue
