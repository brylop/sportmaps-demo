...
-- This function will now count from enrollments (the source of truth)
CREATE OR REPLACE FUNCTION public.sync_program_participant_count()
RETURNS TRIGGER AS $$
DECLARE
    v_program_id uuid;
BEGIN
    -- Determine which program_id to update
    ...
    -- Also update OLD program if it was changed
    IF (TG_OP = 'UPDATE' AND OLD.program_id IS DISTINCT FROM NEW.program_id AND OLD.program_id IS NOT NULL) THEN
        UPDATE public.programs
        SET current_participants = (
            SELECT count(*) 
            FROM public.enrollments 
...
CREATE TRIGGER trg_sync_program_participants_on_enrollments
