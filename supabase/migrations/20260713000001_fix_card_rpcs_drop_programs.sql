-- ============================================================
-- SPORTMAPS — Carnets: quitar referencia a la tabla legacy `programs`
-- ------------------------------------------------------------
-- Las RPCs de carnets (my_athlete_id_cards, verify_athlete_id_card_public,
-- list_athlete_id_cards) tenían un subquery de fallback para el branch_name
-- del atleta ADULTO vía enrollments → programs → school_branches. `programs`
-- es tabla legacy (reemplazada por offerings/teams) y NO existe en la BD real,
-- por lo que ejecutar estas funciones fallaba con
-- "relation public.programs does not exist" (rompía la página de Carnets).
--
-- Fix: el fallback ahora resuelve el branch vía enrollments → teams → branch
-- (teams.branch_id existe). Idéntico en lo demás. Ver [[project_qr_inscripcion_flow]].
-- Fecha: 2026-07-13
-- ============================================================

-- 1. my_athlete_id_cards
...
-- 2. verify_athlete_id_card_public
