...
AS $$
  SELECT COALESCE(ARRAY_AGG(school_id), ARRAY[]::uuid[]) FROM public.school_members
...
DO $wrap$ BEGIN IF EXISTS (SELECT FROM pg_tables WHERE schemaname='public' AND tablename='classes') THEN EXECUTE 'CREATE POLICY "classes_select_school"
ON classes FOR SELECT
USING (school_id IN (SELECT public.user_school_ids()));'; END IF; END $wrap$;

DO $wrap$ BEGIN IF EXISTS (SELECT FROM pg_tables WHERE schemaname='public' AND tablename='classes') THEN EXECUTE 'CREATE POLICY "classes_write_staff"
ON classes FOR ALL
USING (
  school_id IN (
    SELECT school_id FROM school_members
...
DO $wrap$ BEGIN IF EXISTS (SELECT FROM pg_tables WHERE schemaname='public' AND tablename='carts') THEN EXECUTE 'CREATE POLICY "carts_own_only"
ON carts FOR ALL
...
DO $wrap$ BEGIN IF EXISTS (SELECT FROM pg_tables WHERE schemaname='public' AND tablename='school_settings') THEN EXECUTE 'CREATE POLICY "settings_school_staff"
...
DO $wrap$ BEGIN IF EXISTS (SELECT FROM pg_tables WHERE schemaname='public' AND tablename='academic_progress') THEN EXECUTE 'CREATE POLICY "academic_progress_school_or_parent"
...
DO $wrap$ BEGIN IF EXISTS (SELECT FROM pg_tables WHERE schemaname='public' AND tablename='academic_progress') THEN EXECUTE 'CREATE POLICY "academic_progress_write_coach"
ON academic_progress FOR ALL
USING (coach_id = auth.uid());'; END IF; END $wrap$;

-- ─── PASO 15: calendar_events ────────────────────────────────────────────────
DO $wrap$ BEGIN IF EXISTS (SELECT FROM pg_tables WHERE schemaname='public' AND tablename='calendar_events') THEN EXECUTE 'CREATE POLICY "calendar_own_events"
...
-- attendance_records: columna de fecha se llama 'attendance_date' y estudiante 'child_id' en el esquema final
CREATE INDEX IF NOT EXISTS idx_attendance_records_school_student
  ON attendance_records (school_id, child_id);
...
CREATE INDEX IF NOT EXISTS idx_attendance_records_student_date
  ON attendance_records (child_id, attendance_date DESC);

-- attendance (tabla simple) eliminada en la refactorización a favor de attendance_records
...
-- (el schema final ya utiliza el enum pay_status completo, por lo que este drop/add no es necesario)
