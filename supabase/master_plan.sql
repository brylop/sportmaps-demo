...
-- MIGRATION: Ensure attendance_records has the correct column names
DO $$
BEGIN
    -- Rename 'student_id' to 'child_id' if it exists
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attendance_records' AND column_name = 'student_id') THEN
        ALTER TABLE public.attendance_records RENAME COLUMN student_id TO child_id;
    END IF;

    -- Rename 'date' to 'attendance_date' if it exists
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attendance_records' AND column_name = 'date') THEN
        ALTER TABLE public.attendance_records RENAME COLUMN "date" TO attendance_date;
    END IF;
END $$;
...
CREATE OR REPLACE VIEW public.children_full AS
  SELECT ch.*,
         t.name       AS team_name_view,
         t.sport      AS team_sport_view,
         pr.name      AS program_name_view,
         sb.name      AS branch_name_view
...
CREATE POLICY "School staff view school children" ON public.children FOR SELECT USING (school_id = ANY(public.user_school_ids()));
...
CREATE POLICY "School admin update school children" ON public.children FOR UPDATE USING (public.is_school_admin(school_id));
...
CREATE POLICY "School staff view school enrollments" ON public.enrollments FOR SELECT USING (school_id = ANY(public.user_school_ids()));
...
CREATE POLICY "School staff view school payments" ON public.payments FOR SELECT USING (school_id = ANY(public.user_school_ids()));
...
CREATE POLICY "School staff manage attendance" ON public.attendance_records FOR ALL USING (school_id = ANY(public.user_school_ids()));
