...
**F4 (frontend del informe grupal) — aplicado 2026-09-14:** `frontend/src/pages/CoachTeamPostTrainingReportPage.tsx` (ruta `/coach-reports/entreno-equipo`) + 4 rutas nuevas en `bff/src/routes/school/reports.ts` (`GET .../team/:teamId/preview` con find-or-create del borrador, `POST .../team/:teamId/publish`, `PUT .../team-section-note`). Vista de coach/admin: selector de equipo/mes/año, tarjetas por métrica (avg/distribution/count), nota por bloque, botón de publicar. La versión del padre (§5.3, su hija sobrepuesta al grupo) y "compartir"/export a imagen siguen sin construir.
...
**Bug crítico encontrado y corregido al probar el cron contra la base viva (2026-09-14):** `generate_team_report_drafts_system()` (migración `20260914151925`) fallaba con `column reference "school_id" is ambiguous` para **todas** las escuelas, siempre — el OUT param `school_id` de la función colisiona con la columna homónima del `ON CONFLICT (school_id, ...)` sobre `team_reports`. Fix en `20260914193611_fix_team_report_drafts_ambiguous_column.sql` (directiva `#variable_conflict use_column`, sin cambiar la firma). **Esta migración quedó commiteada en el repo pero NO aplicada a la base compartida** — el sistema de permisos bloqueó la aplicación en vivo por tratarse de un recurso compartido; alguien con acceso debe aplicarla (`supabase db push` o `apply_migration`) antes de que el cron pueda generar borradores nuevos.

**Mismo bug, sin tocar, en otro módulo — reportar aparte:** `generate_report_drafts_system()` (migración `20260814173709`, el ciclo diario del informe INDIVIDUAL del atleta, en producción desde el 14 de agosto) tiene el idéntico patrón (`OUT school_id` + `ON CONFLICT (school_id, ...)` sobre `athlete_reports`) y el mismo error, confirmado en la base viva: **todas** las escuelas devuelven `error_msg` ambiguo y `created=0` siempre. En la práctica, el cron del informe individual lleva un mes sin crear NINGÚN borrador nuevo por esta vía (los borradores que sí existen se crearon por otro camino, ej. a mano). Deliberadamente fuera del alcance de este ticket (es el módulo "Informe Mensual del Atleta", no "Evaluación Post-Entrenamiento") — mismo fix aplicaría (`#variable_conflict use_column`).
- Falta la versión del padre del informe grupal (§5.3) y "compartir"/export a imagen.
- Sin tests unitarios (vitest) para `report-snapshot.service.ts` — cubierto por E2E real + `tsc --noEmit`, no por suite unitaria.
