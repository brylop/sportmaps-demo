...
**F4 (frontend del informe grupal, vista de coach/admin) — aplicado 2026-09-14:** `frontend/src/pages/CoachTeamPostTrainingReportPage.tsx` (ruta `/coach-reports/entreno-equipo`) + rutas en `bff/src/routes/school/reports.ts` (`GET .../team/:teamId/preview` con find-or-create del borrador, `POST .../team/:teamId/publish`, `PUT .../team-section-note`). Vista de coach/admin: selector de equipo/mes/año, tarjetas por métrica (avg/distribution/count), nota por bloque, botón de publicar.

**F4 (vista del padre del informe grupal) — aplicado 2026-09-14:** resuelve §7 abierta #1 hacia "sí, el padre puede verlo" — pero acotado. Nueva ruta `GET .../team/:teamId/preview-for-parent?child_id&year&month`, sin `requireRole(STAFF)`: la autorización es de negocio (RLS de `children` confirma que es su hijo/a + inscripción activa en el equipo), y **solo expone el informe si ya fue publicado** (mismo criterio que el individual, spec §8.4 — un borrador que el coach puede seguir corrigiendo no se le muestra a la familia). Devuelve el agregado del equipo + las métricas puntuales de esa hija (`loadSessionMetrics`, exportada para esto) + únicamente la nota general del coach (nunca las notas por bloque, que son herramienta de trabajo del coach). Pantalla `frontend/src/pages/ChildPostTrainingReportPage.tsx`, ruta `children/:id/post-entreno-informe`. Probado contra la base viva: 404 con informe en borrador, 403 con hijo ajeno o sin inscripción activa, 200 con datos correctos en el camino feliz.

**F4 (PDF exportable) — aplicado 2026-09-14:** `GET .../team/:teamId/pdf?year&month` — el objetivo original del módulo (reemplazar el PDF manual de Besser). Si el informe ya está publicado lee el `snapshot` congelado (nunca recalcula); si no, lo arma en vivo y marca "BORRADOR — SIN PUBLICAR". Reusa branding de la escuela y el patrón ya probado del PDF individual (`athlete-reports-pdf.ts`, refactorizado para compartir helpers vía `bff/src/utils/reportPdfHelpers.ts` sin cambiar su comportamiento). Botón "Descargar PDF" en la pantalla del coach. Probado generando PDFs reales contra la base viva (equipo Thunder, con y sin publicar).

**Sigue sin construir:** la versión "compartir a imagen" (tarjeta tipo redes sociales) mencionada en el diseño de UX — nunca se decidió si vale la pena para este alcance.

**E2E real, contra la base compartida — 2026-09-14:** `frontend/e2e/post-entreno.spec.ts` + fixture `supabase/seed/post_entreno_test_users.sql` (usuarios `qa-post-entreno-parent@sportmaps.test` / `qa-post-entreno-coach@sportmaps.test`, password `TestPass123!`, en "Escuela Demo SportMaps" / equipo Thunder — **no se tocó ninguna escuela ni cuenta real**). **2 de 2 tests pasan**: el padre completa las 5 preguntas y ve "¡Listo!" con su racha y promedio; el coach ve el agregado del equipo (BORG=9, esfuerzo=90%, etc.) en el informe grupal recién construido. Requiere BFF local en `:3000` — `playwright.config.ts` compartido apunta a un túnel ngrok vía `VITE_BFF_URL` (para pruebas desde celular) cuyo CORS no admite `localhost`, así que se corre con `playwright.postentreno.config.ts` dedicado (ver comentario en el archivo).

**Deuda resuelta 2026-09-14 — cron del informe grupal:** `bff/src/jobs/team-reports.job.ts` (`runTeamReportsCycle`) genera los borradores del mes con `generate_team_report_drafts_system()` y publica los que ya cumplieron su día de envío, reusando la MISMA cadencia del informe individual (`report_team_schedule.send_day` / `school_settings.reports_default_send_day`, vía las funciones `_report_send_day` / `_report_scheduled_for` de la migración `20260814173709`) — no una fecha nueva. Registrado en `maintenance.job.ts` a las 06:12 COT, dos minutos después del ciclo individual (mismo kill-switch `DISABLE_ATHLETE_REPORTS_CRON`). Sin paso de envío: el informe de equipo no se manda a familias (spec §7 abierta #1 sigue sin decidir). El botón "Publicar informe" sigue disponible para adelantar la publicación antes de que llegue el día.

**Bug crítico encontrado Y APLICADO a la base compartida (2026-09-14):** `generate_team_report_drafts_system()` (migración `20260914151925`) fallaba con `column reference "school_id" is ambiguous` para **todas** las escuelas, siempre — el OUT param `school_id` de la función colisiona con la columna homónima del `ON CONFLICT (school_id, ...)` sobre `team_reports`. Fix en `20260914193611_fix_team_report_drafts_ambiguous_column.sql` (`#variable_conflict use_column`, sin cambiar la firma). Verificado post-fix contra la base viva: `error_msg=null` en todas las escuelas.

**Bug gemelo en OTRO módulo, encontrado Y TAMBIÉN APLICADO (2026-09-14):** `generate_report_drafts_system()` (migración `20260814173709`, el ciclo diario del Informe MENSUAL DEL ATLETA, en producción desde el 14 de agosto) tenía el idéntico patrón (`OUT school_id` + `ON CONFLICT (school_id, ...)` sobre `athlete_reports`). Confirmado en la base viva ANTES del fix: **todas** las escuelas devolvían `error_msg` ambiguo, `created=0`, siempre — el cron del informe individual llevaba **~1 mes sin crear ningún borrador nuevo** por esta vía (los que existían se crearon por otro camino, ej. a mano). Fix en `20260914194842_fix_generate_report_drafts_ambiguous_column.sql`, mismo patrón, aplicado y verificado (`error_msg=null` en todas las escuelas). Este es un incidente del módulo "Informe Mensual del Atleta", no de este spec — se documenta acá porque se encontró como efecto colateral de cablear el cron gemelo de este módulo.

**Tests unitarios (vitest) — agregados 2026-09-14:** `bff/src/services/report-snapshot.service.test.ts`, 8 tests. Cubre la regla más frágil del módulo (comprensión y satisfacción con `aggregation='distribution'` NUNCA se promedian), `avg`/`count` correctos, y los casos borde (cero mediciones, métrica sin catálogo, `aggregation='latest'` excluida de `metrics_session`). `loadSessionMetrics` pasó a `export` para poder testearla directo.

**Opt-out con interfaz — agregado 2026-09-14:** toggle en `frontend/src/components/settings/NotificationsSection.tsx` (atletas adultas, reusa la RPC `save_notification_preferences` que ya mergea el jsonb) y en `frontend/src/pages/ChildProgressPage.tsx` (menores, `UPDATE` directo a `children` que la RLS `"Children: update parent"` ya permite). Sin RPC ni migración nueva — el campo y el trigger que lo respeta ya existían desde F2, solo faltaba la interfaz.

**Sigue sin construir:** la versión "compartir a imagen" — nunca se decidió si vale la pena para este alcance.
