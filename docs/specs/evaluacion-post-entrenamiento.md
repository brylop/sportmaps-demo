...
- No hay job de cron que llame `generate_team_report_drafts_system()` / `publish_team_report_system()` automáticamente — evité inventar una cadencia de publicación (mensual, "última semana") sin una decisión de producto real detrás; publicarlo prematuramente a mitad de mes sería peor que dejarlo manual por ahora. El botón "Publicar informe" de la pantalla ya lo hace a demanda.
