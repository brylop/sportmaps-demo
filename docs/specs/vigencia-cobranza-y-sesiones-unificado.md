...
|---|---|---|---|
| **0** | Confirmar D1-D10 con el dueño de producto. Documentar (versionar) `fn_expire_overdue_enrollments` y `fn_expire_overdue_payments` en migración nueva **sin cambiar comportamiento todavía** — sacarlas del drift. **CONSTRUIDO 2026-09-02** — migración `20260902171533` (`fn_expire_overdue_payments` ya estaba versionada desde `20260804125644`). Solo se confirmó D1 con el usuario, no D1-D10 completo. | — | Fase 0 cobranza |
| **1** | **Bloqueador B** (Bug del trigger, D5) en una sola migración: fix elegido + corrección puntual de las 10 filas con `expires_at` ya vencido + las 77 con pagos ciegos. Debe ir antes que la Fase 2 porque §3.3 lo exige. **CÓDIGO CONSTRUIDO 2026-09-02 — falta la corrección de las filas históricas, ver nota abajo.** | Fase 0 (D5) | (nuevo, no estaba en ninguno de los dos originales) |
...
> **⚠️ Orden invertido 2026-09-02 — leer antes de asumir que esto ya está resuelto.** Se construyó la Fase 2 (Bloqueador A, D1 = híbrido con ventana de 7 días — migración `20260902171932`) **sin haber resuelto antes la Fase 1 (Bloqueador B)**, contradiciendo la advertencia explícita de §3.3: *"el fix de A solo pospone el problema unos días en vez de resolverlo — seguiría cancelando a alguien al día, más tarde."* Al medir el radio se confirmó en vivo: al menos 3 inscripciones activas (escuelas `57ba9352…`, `5ddf91c2…`, `90b2cc2e…`) con pago real nunca extendido por el trigger ciego, vencidas desde **febrero/mayo/agosto 2026** — meses, no días — ya más allá de la ventana de 7 días. El cron de esa misma noche (2026-09-02, 03:00 COT) las habría cancelado igual.
>
> **Acción tomada: el cron `expire-overdue-enrollments` (job_id 2) está PAUSADO (`active=false`) desde 2026-09-02** — migración `20260902172435`, decisión explícita del usuario (opción elegida: pausar en vez de data-fix puntual o dejarlo correr). **Mientras esté pausado, NINGUNA inscripción se cancela automáticamente por vigencia vencida** — ni las 3+ del blind spot, ni las genuinamente impagas sin ningún bug de por medio. La única red de cobranza mientras tanto es la que ya existía antes de este spec: `OverdueAccountsCard`/`PaymentAgingCard` y el badge de mora en asistencia (2.4, 2.6) — ninguna de las dos cancela nada, solo muestran.
>
> **REACTIVAR** (`SELECT cron.alter_job((SELECT jobid FROM cron.job WHERE jobname='expire-overdue-enrollments'), active := true);`, en una migración nueva, no a mano) **solo después de que la Fase 1 (Bloqueador B) esté construida** — reactivarlo antes reproduce exactamente el riesgo que motivó la pausa. El fix de A (grace + pausa, ya construido) no es incorrecto ni hay que revertirlo; simplemente no alcanza solo.
>
> **Actualización 2026-09-02 — código de Bloqueador B construido, migración `20260902174423`, D5 confirmado (arreglar las 3 rutas, no aflojar el trigger):**
> - Trigger `trg_extend_enrollment_on_payment_paid` ahora `AFTER INSERT OR UPDATE` (antes solo `UPDATE`) — verificado en vivo con INSERT directo como `'paid'` dentro de una transacción con ROLLBACK: `expires_at` se extendió correctamente, sin dejar rastro.
> - `generate_qr_monthly_charge()` ahora setea `offering_plan_id` desde la inscripción activa del atleta — verificado en vivo (mismo patrón de transacción + ROLLBACK): el pago nuevo salió con el `offering_plan_id` correcto.
> - `RegisterCashPaymentModal.tsx` y `recurring-charges.service.ts` (autopay) ahora setean `offering_plan_id` en el INSERT — compilan limpio (`tsc --noEmit`), **sin probar en vivo** (BFF local no levanta por la credencial de MercadoPago en `bff/.env`, pendiente desde antes en este mismo repo).
>
> **Actualización 2026-09-02 (2) — backfill histórico, parcial a propósito.** Censo completo en vivo: 135 inscripciones afectadas (8 escuelas, 95 atletas distintos — más que el estimado original de 77, censo más amplio). Se dividieron en dos grupos:
> - **72 no ambiguas** (el atleta tiene UNA sola inscripción con plan en esa escuela) — corregidas en migración `20260902175555`: se reprodujeron sus pagos `'paid'` reales en orden cronológico con la misma fórmula del trigger (`GREATEST(expires_at_previo, fecha_del_pago) + duration_days`). 61 de las 72 cambiaron de valor (las otras 11 ya daban el mismo resultado — su pago ciego quedó cubierto por un pago posterior que sí disparó bien). Verificado: `61` filas con `updated_at` fresco tras aplicar.
> - **63 ambiguas — sin tocar, documentadas acá para revisión manual:** el atleta tiene 2+ inscripciones con plan en la misma escuela, y un pago histórico sin `offering_plan_id` no se puede atribuir con certeza a cuál de los dos pertenece. Adivinar ahí es peor que dejarlo desactualizado. Desglose:
>
>   | Escuela | Inscripciones ambiguas | Atletas distintos |
>   |---|---|---|
>   | Dynasty Volley Club | 31 | 15 |
>   | MMA Blair Team | 27 | 6 (~4.5 planes/atleta — típico de escuela de artes marciales con plan por disciplina/nivel) |
>   | Academia Superior Bogotá | 3 | 1 |
>   | Escuela Demo SportMaps | 2 | 1 |
>
>   Hallazgo colateral al investigar un caso ambiguo (MMA Blair Team, atleta con plan "Plan david Lopez" + "Equipo COMBATE MMA"): ~15 filas de `payments` con `status='cancelled'`, `payment_date` NULL, generadas automáticamente cada noche a la 01:30 durante dos semanas de julio, mismo monto/concepto — parece un bug separado (algún proceso de reintento/recordatorio duplicando intentos cancelados). **No investigado ni corregido — fuera de alcance de Bloqueador A/B, queda para otra sesión.**
>
> **Actualización 2026-09-02 (3) — Dynasty resuelto, migración `20260902180236`.** Usuario: "dynasty son las mas importantes". Revisión caso por caso de los 15 ambiguos de Dynasty:
> - **13 eran el mismo patrón de [[project_f0_duplicate_enrollments_audit]]** (inscripción duplicada huérfana — un UPDATE que debía asignar equipo creó una fila nueva en su lugar, dejando la vieja cancelada y sin equipo). La inscripción `'active'` es siempre la real. Corregidas reproduciendo pagos reales, mismo método que las 72.
> - **2 eran cambios de plan genuinos** (Gabriela Soracá Plaza, Solangye Andulce Morales Morales) — sin patrón automático confiable. Por decisión del usuario, se extendió la inscripción activa desde HOY (no se reprodujo historial completo, que mezclaría pagos de dos planes distintos).
> - Los 3 enrollments "urgentes" detectados el mismo día: 1 de 3 pertenecía a Dynasty y ya quedó resuelto (ahora vence 2026-10-22); los otros 2 (uno en MMA Blair Team, ambiguo y sin tocar; otro en una escuela sin identificar) siguen expuestos — no eran parte del alcance "Dynasty" de esta vuelta.
> - **Hallazgo nuevo, sin investigar:** 15 inscripciones activas de Dynasty, TODAS con `expires_at = 2026-08-05` (fecha idéntica, sospechoso — no encajan con ningún patrón de Bloqueador B, 11/15 sí tienen algún pago `'paid'` histórico). Podría ser un cohorte real sin renovar, o un artefacto de carga masiva. **No es parte de este fix — queda para revisar en otra sesión.**
>
> **Actualización 2026-09-02 (4) — CERRADO. Cron reactivado, migración `20260902181550`.**
> - **Resto de ambiguos (MMA Blair Team, Academia Superior Bogotá, Escuela Demo) — migración `20260902181143`.** Guarda de seguridad nueva aprendida acá: reproducir el historial completo de pagos de 4/6 atletas de MMA Blair Team daba un `expires_at` MENOR al ya guardado (ej. david Lopez: guardado 2026-08-17, recomputado 2026-07-24) — señal de que el valor actual ya reflejaba algo que el replay no ve (probable corrección manual de un admin). Se agregó `GREATEST(expires_at_actual, recomputado)` — el backfill solo sube, nunca baja. Con esa guarda: 2 cambios reales (Carlos Martinez Perez, cuenta de prueba), 4 sin cambio (ya estaban bien), David Junior Andrade (Academia Superior, 3 planes reales) extendido desde hoy igual que los 2 casos especiales de Dynasty, "pruebas" (MMA Blair) sin inscripción activa — nada que corregir.
> - **Auditoría retroactiva de las migraciones `175555` + `180236`** (73 filas): recomputadas hoy, 71/73 dan el mismo valor, las 2 distintas son exactamente Gabriela Soracá Plaza y Solangye (los 2 casos ya tratados aparte a propósito). Cero filas dañadas por el trabajo anterior.
> - **Las 16 inscripciones con `expires_at` idéntico (2026-08-05) — investigadas, NO es un bug.** 15 son de Dynasty, todas creadas en la misma ventana de ~25 minutos el 2026-07-06 (lote de inscripción masiva) con `expires_at = created_at + 30` de fábrica; ninguna pagó ni renovó desde entonces. La 16ª es un atleta sin cuenta de GYM RM, caso aislado. Candidatas legítimas a cancelar — mismo tratamiento que cualquier vencido real.
> - **Cron `expire-overdue-enrollments` REACTIVADO 2026-09-02** (`active=true`, migración `20260902181550`) — decisión explícita del usuario tras confirmar que no queda ningún caso de Bloqueador A/B sin resolver. Vuelve a correr 08:00 UTC / 03:00 COT con grace period + ventana de 7 días + exclusión de pausados. Las ~58 inscripciones genuinamente vencidas de la plataforma (incluidas las 16 del lote) se cancelarán en las próximas corridas si nadie renueva antes.
>
> **Bloqueador A y B: CERRADOS.** Pendiente real para más adelante: Fase 2 (escalón `overdue_1d/3d/7d` multicanal, para que el aviso de los 7 días de gracia sea activo y no solo pasivo) y Fase 3+ (bandeja de cobranza, informe semanal) — ver §7.
