...
- **D1 — RESUELTO 2026-09-02: híbrido.** Sigue cancelando automático, pero con 7 días de ventana tras `payment_grace_days` (mismo umbral que `overdue_7d`) antes de cerrar — construido en `20260902171932`. Nota de implementación: el "aviso" de esos 7 días hoy es **pasivo** (badge de asistencia, `OverdueAccountsCard`, `payment_status`) — el aviso **activo** multicanal (`collection_notices`/email/push en cada escalón) sigue siendo Fase 2, 0% construida. Si Fase 2 tarda, la ventana de 7 días igual protege contra el cierre-sin-aviso que motivó este bloqueador.
...
- **D4 — RESUELTO 2026-09-10: ambos caminos.** El acudiente (y el atleta adulto) **solicita** la pausa desde su módulo principal y el admin la **aprueba**; el admin además puede pausar directo con un botón que hace toda la acción, sin solicitud previa (mismo patrón de UX que el botón de becado). Plan completo, con las decisiones D5/D6/D7 que salieron de ahí (granularidad por mes completo, anulación de los cobros ya emitidos del periodo pausado, rollout opt-in por escuela), en **`docs/specs/pausa-vacaciones-enrollments.md`**. Ese spec manda sobre la §4.1 de este documento en todo lo que se refiere a la pausa.
...
|---|---|---|
| 0 | **Versionar `fn_expire_overdue_enrollments` y `fn_expire_overdue_payments`** en migración nueva (sin cambiar comportamiento todavía) — saca ambas del drift. Confirmar D1-D4 con el usuario. **CONSTRUIDO 2026-09-02** — `fn_expire_overdue_payments` ya estaba versionada (`20260804125644`, y su cron desagendado ahí mismo, redundante con `apply_late_fees`); solo `fn_expire_overdue_enrollments` era drift real. Migración `20260902171533`: mismo cuerpo, cero cambio de comportamiento, mismos GRANTs (`service_role` únicamente), `COMMENT ON FUNCTION` documentando el bug conocido para que no se pierda de vista. **D1 sigue pendiente de confirmar con el usuario antes de escribir la Fase 1.** | Sin esto, cualquier fase siguiente se construye sobre arena: el fix real vive en la base, no en el repo. |
| 1 | Fix de `fn_expire_overdue_enrollments`: respeta `payment_grace_days` y excluye pausados. Agregar columnas de pausa a `enrollments`. **CONSTRUIDO 2026-09-02** — migración `20260902171932`. Radio medido antes de aplicar: 0 inscripciones activas caían hoy en la ventana afectada (el cron ya había barrido todo lo que la lógica vieja alcanzaba a cancelar), así que el cambio es puramente hacia adelante. `apply_late_fees()` queda **sin tocar a propósito** — excluir pausados ahí exige matchear `payments` con `enrollments` por identidad de atleta (no hay `enrollment_id` en `payments`), y es la función que aplica recargos reales a 368 escuelas; se retoma cuando exista UI de pausa (hoy `paused_reason` es 100% NULL, nadie puede pausar todavía). | Resuelve el bloqueador crítico antes de tocar notificaciones. |
