...
| **INF-12** | 🟢 **Puente físico de apertura remota — GYM RM (ZKTeco F22ID). Código completo en `develop`, falta desplegar.** El F22ID acepta el comando ADMS de apertura (`CONTROL DEVICE`, responde `Return: 0`) pero **no mueve el relé físico** — bug de firmware acotado a ese comando puntual (asistencia y el resto de comandos `DATA UPDATE USERINFO` sí funcionan). El bloqueo de usuarios (`set_group`→`Grp=2`) **se probó en campo el 2026-08-25/26 y sí funciona por ADMS** — se queda ahí, no se extiende el bridge a eso: es la vía con menos partes que pueden fallar, y ya tiene reconciliación ante un reset del torniquete vía el historial de `device_commands` que lee `GET /api/v1/access/overdue`. Serial↔dirección e IPs confirmados contra el formulario "Editar dispositivo" real (`JJA1254900899`=entrada `192.168.1.6`, `JJA1254900898`=salida `192.168.1.7` — coincide con `door_bridge.py`; el doc de junio era el desactualizado). **Implementado 2026-08-26:** migración `20260825231925_gymrm_bridge_local_flag.sql` (`turnstile_devices.has_local_bridge` + `device_commands.claimed_at`, evita del todo la duda sobre el CHECK de `status`); `access-adms.ts` (`getrequest`) ya excluye `open_door` para dispositivos con bridge, cerrando la carrera que marcaba comandos "ejecutados" en falso; `bff/src/routes/bridge.routes.ts` nuevo con `GET/POST /bridge/door-commands` (API key fail-closed, mismo patrón que `internal-notifications.routes.ts`) — el spec original embebía `turnstile_devices(...)` en el select de Supabase asumiendo una FK que **no existe** entre `device_commands.device_id` y `turnstile_devices`; se resolvió con dos queries. El tiempo de apertura ya no está hardcodeado: el endpoint reenvía `turnstile_devices.door_drive_time_seconds`, el mismo campo que edita el dashboard. `tsc --noEmit` y `migrations:check` en verde. **Falta, fuera del alcance de una sesión sin credenciales de Supabase/Render:** aplicar la migración a la base real, desplegar el BFF, setear `BRIDGE_API_KEY` en Render, instalar `door_bridge.py` en la PC del gym, y la prueba física end-to-end. | 🟢 código listo | falta solo desplegar | [validación](../scripts/gymrm-door-bridge/VALIDACION-2026-08-25.md) · [spec](../scripts/gymrm-door-bridge/BACKEND_ENDPOINT_SPEC.md) · [bridge](../scripts/gymrm-door-bridge/) · [migración](../supabase/migrations/20260825231925_gymrm_bridge_local_flag.sql) |
