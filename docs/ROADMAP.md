...
| **SEG-19** | 🟡 **Fase B de SEG-15: los RPC `SECURITY DEFINER` saltan RLS.** Las policies RESTRICTIVE de la Fase A (aplicada el 08-17) no los alcanzan: corren como su dueño. Y **las siete se llaman DIRECTO desde el frontend a Supabase**, no por el BFF, así que el 402 del middleware tampoco las cubre — verificado: `create_invitation` desde 5 pantallas, `submit_qr_signup` desde la página pública, `create_school_join_qr` desde el panel. **Exposición medida el 2026-08-17**, que reordena la prioridad: `create_invitation` es la única con superficie viva — **36 invitaciones pendientes de escuelas bloqueadas**, y una bloqueada puede seguir emitiendo. El QR **no tiene superficie hoy**: 9 códigos y los 9 inactivos, aunque `create_school_join_qr` puede crear uno nuevo y reabrir el camino. Los certificados **no se usan**: `athlete_certificates` tiene 0 filas. `notify_user` es ruido, no dinero. ⚠️ Ponerles el guard exige `CREATE OR REPLACE`, que reemplaza el cuerpo entero, y hoy ya se vio dos veces que el repo no coincide con la base (INF-1): primero hay que sacar los cuerpos vivos con [`scripts/seg15-faseB-inspeccion.sql`](../scripts/seg15-faseB-inspeccion.sql). | 🟡 | pequeño, después de la inspección | [Fase A](../supabase/migrations/20260813170813_bloqueo_de_prueba_en_rls.sql) |
