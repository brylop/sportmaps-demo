...
| **SEG-15** | **El bloqueo de fin de prueba no llega a RLS.** Es la mitad viva de `DIN-4`. El middleware del BFF cubre lo que pasa por el BFF, pero el navegador escribe **directo a Supabase en 52 sitios** más los RPC, así que una escuela inhabilitada todavía puede escribir por esa vía. Fix escrito con policies **`RESTRICTIVE`** —se combinan con `AND` sobre las permissive existentes sin modificar ninguna, y se revierten con `DROP POLICY`— sobre 14 tablas con `school_id`, **solo en INSERT/UPDATE/DELETE** (nunca SELECT: bloqueado no es «sin datos»). No enciende RLS donde no está: reporta las tablas que quedan descubiertas en vez de fingir cobertura. ⚠️ Falta después: los RPC `SECURITY DEFINER` que escriben saltan RLS por definición (`submit_qr_signup`, `create_invitation`, `create_school_join_qr`, `generate_qr_monthly_charge`, certificados, `notify_user`) — cada uno necesita su guard. | 🟡 | migración escrita, sin aplicar | [migración](../supabase/migrations/20260813170813_bloqueo_de_prueba_en_rls.sql) |
...
| **CAR-4** | **Membresías del club** — tabla `memberships` deliberadamente **fuera de facturación** (sin montos, sin FK a pagos, ningún cron la mira). Estado declarado por Carmel: manual o CSV hoy, API después vía `external_ref`. Decisión firme: `valid_until` **no vence solo** (un vencimiento automático con dato rezagado crea suspendidos fantasma) y **el biométrico no la lee** en el trial. | 🔵 | pequeño | [plan](plan-club-carmel-multideporte-2026-08-15.md) §2 |
...
| **INF-8** | 🔴 **Nada impide desplegar un `ReferenceError`.** `vite build` transpila con esbuild y **no typechequea**, y `npm run build` tampoco corre `tsc`. El `tsc` completo tarda **>10 minutos** y arroja **~300 errores preexistentes** (el grueso son los tipos generados de Supabase, desactualizados: no conocen `sports_categories.slug`, `vendor_profiles` ni `service_listings`), así que **dos errores nuevos se pierden en el ruido**. Costó caro el 2026-08-17: mover el catálogo de deportes a la base dejó tres referencias muertas y las tres se desplegaron — la página de **Equipos** entera caída por zona muerta temporal (`TS2448`), el **registro de una escuela** imposible (`TS2304` + `sportId: z.number()` contra un uuid) y el formulario de planes de **Ofertas** reventando (`TS2552`). Mitigación ya en el repo: `npm run verificar:runtime` filtra `tsc` a los cuatro códigos que son fallas de ejecución garantizadas (2304, 2552, 2448, 2449) — cero falsos positivos, sirve como gate antes de pushear. **Lo que falta es lo de fondo:** regenerar los tipos de Supabase para bajar el ruido, y recién entonces poner `tsc` en pre-commit o CI. Mientras el ruido siga, el gate acotado es lo único que se puede exigir. | 🟡 | gate hecho · limpieza mediana | [gate](../scripts/verificar-nombres-sin-declarar.mjs) |
