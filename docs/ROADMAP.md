...
| **SEG-19** | **Fase B de SEG-15: los RPC `SECURITY DEFINER` saltan RLS.** Las policies RESTRICTIVE de la Fase A no los alcanzan —corren como su dueño— y las siete se llaman **directo desde el frontend a Supabase**, no por el BFF, así que el 402 del middleware tampoco. **Exposición medida el 2026-08-18:** 9 QR de inscripción y **los 9 ACTIVOS**, uno de ellos de una escuela bloqueada (Fit And Fight, 0 inscripciones); más **36 invitaciones pendientes** de escuelas bloqueadas. ⚠️ La medición del 08-17 dijo «los 9 inactivos» y era **falsa**: consultaba `is_active`, columna que no existe —se llama `active`—, la consulta fallaba y el script leía el error como cero. **Una consulta que falla no es un cero**, el mismo patrón que hizo declarar cerrado a `SEG-14`. ✅ **Hecho para las dos del QR** (`20260818071427`): en vez de `CREATE OR REPLACE` —que obliga a reproducir las ~250 líneas de deduplicación de identidades de `submit_qr_signup`— se **renombra** la original a `__interno`, se crea un envoltorio con el mismo nombre y firma que hace el guard y delega, y al interno se le quita `authenticated`. El cuerpo original queda byte por byte igual. ⏳ **Falta `create_invitation`** (la de superficie viva) y las de certificados y `notify_user`, que no tienen uso: `athlete_certificates` está en 0 filas. Para esas hace falta su firma real, con la misma inspección. | 🟡 | QR listo, falta create_invitation | [migración](../supabase/migrations/20260818071427_guard_de_prueba_en_rpcs_de_qr.sql) |
