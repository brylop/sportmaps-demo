...
| **Seguridad** | 🟡 1 sin verificar → `SEG-8` | Verificado contra la base viva el **2026-08-16**: ~~`SEG-14`~~ **(NO estaba cerrado — reverificado el 2026-08-17, sigue abierto: ver la ficha de SEG-14)**, `SEG-9` (cuatro `/debug-logs` públicos) y `SEG-10` (91 tokens de link de pago + 68 registros de staff a `anon`) **ya están cerrados**. Queda `SEG-8` (41 funciones ejecutables por `anon`), que **no se puede comprobar funcionalmente** —hacerlo sería ejecutar `complete_refund` o `apply_late_fees`— y necesita la verificación por catálogo |
...
| **SEG-14** | 🔴 **CONFIRMADO ABIERTO el 2026-08-17** (el roadmap lo daba por cerrado el 08-16: era falso). `find_athletes_by_document` responde **HTTP 200 a un llamador anónimo** con la llave pública que viaja en el bundle. Probado con un documento real de un menor: devuelve **1 fila** con `full_name`, `date_of_birth`, `school_id`, `school_name`, `team_id`, `team_name`, `branch_name`. O sea: con un número de documento y sin ninguna sesión se obtiene nombre completo, fecha de nacimiento y dónde entrena un menor. ⚠️ **No se puede cerrar a `anon` sin más:** la usa `JoinTeamPage` (`/join-team/:teamId`), la página **pública** de auto-registro del acudiente, donde el padre escribe el documento del hijo ANTES de tener cuenta. Cerrarla rompe el alta. Opciones medidas: (a) exigir `p_school_id` —hoy es opcional y la búsqueda es GLOBAL, todas las escuelas—, que obliga a tener un link de equipo válido; (b) **quitar `date_of_birth`**, que se devuelve pero la pantalla NO muestra: costo cero; (c) enmascarar `full_name` («Carlos S. D.»), suficiente para que el padre reconozca a su hijo y mucho menos para un extraño. | 🔴 | pequeño + decisión de UX | [página](../frontend/src/pages/JoinTeamPage.tsx) |
