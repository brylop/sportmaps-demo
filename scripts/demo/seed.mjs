...
/** El dueño del tenant: quien tenga is_owner en STAFF. */
...
async function stepPeople() {
    ...
    if (val?.autopay) {
        await upsert('payment_tokens', {
            id: duid(`token:${val.key}`), user_id: val.profileId, payment_method_type: 'card',
        ...
        for (const d of DISCIPLINAS) {
            for (const h of d.horarios) {
                ...
                sessions.push({
                    ...
                    created_by: ownerProfile(),
                    finalized_at: off < 0 ? `${date}T${h.end}:00-05:00` : null,
                    finalized_by: off < 0 ? ownerProfile() : null,
                });
            }
        }
    }
        ...
        for (const s of past) {
            for (const m of miembros) {
                ...
                records.push({
                    ...
                    marked_by: coachProfileFor(g.d, g.c) || ownerProfile(),
...
async function stepAccess() {
    ...
    maps.push({ id: duid('zk:porteria'), school_id: SCHOOL_ID, zk_pin: 1, user_id: (people.staff.porteria?.profileId || ownerProfile()) });
    maps.push({ id: duid('zk:gerencia'), school_id: SCHOOL_ID, zk_pin: 2, user_id: ownerProfile() });
    ...
    for (const [i, hora] of ['07:12:00', '18:41:00'].entries()) {
        events.push({
            id: duid(`access:moroso:${TODAY}:${i}`), school_id: SCHOOL_ID,
            ...
            raw_event: { demo: true, zona: zonasAcceso[0] },
        });
    }
    const n = await insertIfEmpty('access_events', events, `access_events?school_id=eq.${SCHOOL_ID}`);
    if (n) ok(`access_events · ${n} registros (últimos 3 días + 2 rechazos de ${daniel.full_name.split(" ")[0]} hoy)`);
}
...
async function stepReservations() {
    ...
    const recepcion = ownerProfile();   // quien registra los alquileres
...
async function stepTorneo() {
    ...
    await upsert('events', {
        ...
        creator_id: ownerProfile(), creator_role: 'school',
    ...
    await upsert('event_registrations', {
        ...
        approved_by: ownerProfile(), approved_at: `${TODAY}T10:00:00-05:00`,
